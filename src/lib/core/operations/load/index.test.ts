...
describe('load operation', () => {
    ...
    it('should load multiple values and set them in context', async () => {
        ...
        await load(params, { context });

        expect(mockGet).toHaveBeenCalledTimes(2);
        expect(mockGet).toHaveBeenCalledWith('http://localhost:3000/resource1', { context });
        expect(mockGet).toHaveBeenCalledWith('http://localhost:3000/resource2', { context });
    ...
    it('should throw error when params is not provided', async () => {
        await expect(load(undefined, { context })).rejects.toThrow(
    ...
    it('should handle get operation errors', async () => {
        ...
        await expect(load(params, { context })).rejects.toThrow('Get operation failed');
    });
});
