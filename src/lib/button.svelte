<script lang="ts">
	...
	interface ButtonProps extends Omit<ElementProps, 'body'> {
		text?: string;
	...
	$: propObject =
		typeof props === 'string' ? ({ text: props } as ButtonProps) : ((props ?? {}) as ButtonProps);
	...
	const Colors: Record<Shape, Record<Color, string>> = {
		filled: {
		...
		outline: {
			...
			danger:
				'bg-transparent border border-destructive text-destructive enabled:hover:bg-destructive/10',
			success:
				'bg-transparent border border-green-500 text-green-500 enabled:hover:bg-green-500/10',
			warning:
				'bg-transparent border border-yellow-500 text-yellow-500 enabled:hover:bg-yellow-500/10',
		...
		ghost: {
			default:
				'bg-transparent border border-transparent text-black shadow-none enabled:hover:bg-gray-50',
			primary:
				'bg-transparent border border-transparent text-primary shadow-none enabled:hover:bg-primary-50',
			danger:
				'bg-transparent border border-transparent text-destructive shadow-none enabled:hover:bg-destructive/5',
			success:
